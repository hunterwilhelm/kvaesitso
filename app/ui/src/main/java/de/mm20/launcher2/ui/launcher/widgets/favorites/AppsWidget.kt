...
fun AppsWidget(widget: AppsWidget) {
    ...
    val selectedTarget by viewModel.selectedTarget.collectAsState(null)
    ...
    Column(
        ...
        if (favorites.isNotEmpty()) {
            ...
            Banner(
                ...
                text = stringResource(
                    if (selectedTarget == null || selectedTarget is SelectorTarget.Favorites) R.string.favorites_empty else R.string.favorites_empty_tag
                ),
                icon = if (selectedTarget == null || selectedTarget is SelectorTarget.Favorites) R.drawable.star_24px else R.drawable.tag_24px,
            )
        }
        if (favoritesEditButton || (!customTags && pinnedTags.isNotEmpty()) || (customTags && widget.config.tagList.size > 1)) {
            FavoritesTagSelector(
                ...
                showRecentlyInstalledApps = !customTags
            )
        }
    }
}